...
public function getProgramsByCenter($center_id)
{
    $programs = DB::table('program_masters as pm')
        ->join('center_managments as cm', 'pm.center_id', '=', 'cm.center_id')
...
public function getProgramsByCenterName($center_name)
{
    $programs = DB::table('program_masters as pm')
        ->join('center_managments as cm', 'pm.center_id', '=', 'cm.center_id')
        ->join('program_creates as pc', 'pm.prog_master_id', '=', 'pc.prog_master_id')
        ->where('cm.center_name', $center_name)
        ->select('pm.program_id', 'pc.program_name', 'pm.number_of_days', 'pm.prog_master_id')
        ->orderBy('pm.program_id', 'asc')
        ->get();
