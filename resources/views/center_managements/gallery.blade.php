                                    ...
                                    <div class="card-body d-flex flex-column justify-content-between">
                                        ...
                                        @if($user && $user->user_type === 'Admin')
                                            ...
                                            <div class="d-flex gap-2 mt-2">
                                                {{-- Replace Form --}}
                                                <form
                                                    action="{{ route('center_managements.gallery.update', [$center->center_id, $loop->index]) }}"
                                                    method="POST" enctype="multipart/form-data" class="flex-fill">
                                                    @csrf
                                                    @method('PUT')

                                                    {{-- Hidden file input --}}
                                                    <input type="file" name="gallery_file" class="d-none"
                                                        id="file-input-{{ $loop->index }}" required>

                                                    <button type="button"
                                                        class="btn btn-sm btn-info w-100 d-flex align-items-center justify-content-center"
                                                ...
                                                {{-- Delete Form --}}
                                                <form
                                                    action="{{ route('center_managements.gallery.delete', [$center->center_id, $loop->index]) }}"
