                        ...
                        <form method="GET" action="{{ url('program-masters') }}" class="mb-3">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <a href="{{ url('program-masters') }}" class="btn btn-primary w-100"  mt-2">Reset Filter</a>
...
document.addEventListener('DOMContentLoaded', function () {
    const centerSelect = document.getElementById('center-select');
    const programSelect = document.getElementById('program-select');
    const selectedProgram = "{{ request('prog_master_id') }}";

    // Load programs on page load if center already selected
    if (centerSelect.value !== '') {
        fetchPrograms();
    }

    centerSelect.addEventListener('change', fetchPrograms);

    // function fetchPrograms() {
    //     const centerName = centerSelect.options[centerSelect.selectedIndex].text;
    //     if (!centerName || centerName === 'Filter by Center') {
    //         programSelect.innerHTML = '<option value="">Filter by Program</option>';
    //         return;
    //     }

    //     fetch('/get-programs-by-center-name/' + encodeURIComponent(centerName))
    //         .then(res => res.json())
    //         .then(data => {
    //             programSelect.innerHTML = '<option value="">Filter by Program</option>';
    //             data.forEach(p => {
    //                 const opt = document.createElement('option');
    //                 opt.value = p.prog_master_id;
    //                 // opt.textContent = `${p.program_name} (${p.number_of_days} days)`;
    //                         opt.textContent = `${p.program_name})`;
    //                 if (selectedProgram == p.prog_master_id) {
    //                     opt.selected = true;
    //                 }
    //                 programSelect.appendChild(opt);
    //             });
...
function fetchPrograms() {
        ...
        .then(data => {
            ...
            data.forEach(p => {
                if (!addedNames.has(p.program_name)) {
                    ...
                    programSelect.appendChild(opt);
                    addedNames.add(p.program_name); // keep track
                }
            });
        })
        .catch(err => {
    ...
    //     if (centerSelect.value !== '') {
    //         // fetchPrograms(centerSelect.value);
    //         fetchPrograms(centerSelect.options[centerSelect.selectedIndex].text);
